...
app.get('/api/leagues-classic/:league_id/standings', async (req, res) => {
  ...
  const params = [];
  if (page_new_entries) params.push(`page_new_entries=${encodeURIComponent(page_new_entries)}`);
  if (page_standings) params.push(`page_standings=${encodeURIComponent(page_standings)}`);
